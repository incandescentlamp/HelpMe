using HelpMe.Infrastructure.TaskQueue.Events;
using HelpMe.Infrastructure.TaskQueue.Internal;
using HelpMe.Infrastructure.TimeCrontab.Constants;
using System.Threading.Channels;

namespace HelpMe.Infrastructure.TaskQueue.Dependencies;
...
internal sealed partial class TaskQueue : ITaskQueue
{
    ...
    /// <param name="runOnceIfDelaySet">配置是否设置了延迟执行后立即执行一次</param>
    /// <returns><see cref="object"/></returns>
    public object Enqueue(Action<IServiceProvider> taskHandler, string cronExpression, int totalMilliseconds, string channel = null, object taskId = null, bool? concurrent = null, bool runOnceIfDelaySet = false)
    {

        return Enqueue(taskHandler, totalMilliseconds, channel, taskId, concurrent, runOnceIfDelaySet);
    }
    ...
    /// <param name="runOnceIfDelaySet">配置是否设置了延迟执行后立即执行一次</param>
    /// <returns><see cref="ValueTask"/></returns>
    //public ValueTask<object> EnqueueAsync(Func<IServiceProvider, CancellationToken, ValueTask> taskHandler, int totalMilliseconds, string channel = null, object taskId = null, bool? concurrent = null, bool runOnceIfDelaySet = false)
    //{
